...
export default function App() {
  ...
  const openProduct = (id) => { setActive(id); go("detail"); };
  ...
  const ProductCard = ({ p }) => {
    ...
    return (
      <div className="press group bg-white rounded-2xl border bd shadowcard overflow-hidden flex flex-col cursor-pointer" onClick={() => openProduct(p.id)}>
        <div className="relative p-3 pb-0">
          ...
          <div className="absolute top-3 left-3 right-3 -m-0 p-0" style={{ pointerEvents: "none" }}>
            <div className="flex items-start justify-between" style={{ pointerEvents: "auto" }}>
              ...
              <button onClick={(e) => { e.stopPropagation(); toggleFav(p.id); }} className="press bg-white/85 backdrop-blur rounded-full p-1.5 shadow-sm" style={{ color: fav ? "var(--up)" : "#9AA1AC" }}>
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          <div className="mt-auto pt-2.5">
            ...
            <button onClick={(e) => { e.stopPropagation(); toggleCompare(p.id); }}
              className="press mt-2 w-full inline-flex items-center justify-center gap-1.5 rounded-xl py-2" style={{ fontSize: 12, fontWeight: 600, color: cmp ? "#fff" : "var(--ink2)", background: cmp ? "var(--ink)" : "#fff", border: cmp ? "1px solid var(--ink)" : "1px solid var(--line)" }}>
  ...
  const DealCard = ({ p }) => {
    ...
    return (
      <div onClick={() => openProduct(p.id)} className="press cursor-pointer bg-white rounded-2xl border bd shadowcard p-3 shrink-0 lg:shrink" style={{ width: isDesktop ? "auto" : 168 }}>
  ...
  const VsCard = ({ a, b }) => (
    <div onClick={() => { setCompare([a.id, b.id]); go("compare"); }} className="press cursor-pointer bg-white rounded-2xl border bd shadowcard p-3 flex items-center gap-2 shrink-0 lg:shrink" style={{ width: isDesktop ? "auto" : 210 }}>
      <Thumb p={a} className="w-11 h-11" radius={11} />
      <span className="mono flex items-center justify-center shrink-0" style={{ color: "#fff", background: "var(--ink)", width: 24, height: 24, borderRadius: 24, fontWeight: 700, fontSize: 10 }}>VS</span>
    ...
    <div className="flex flex-col gap-5">
      <div>
        ...
        <div className="flex flex-col gap-1">
          <button onClick={() => setCat(null)} className="press text-left rounded-lg px-2.5 py-1.5 fontui" style={{ fontSize: 13, background: !cat ? "var(--ink)" : "transparent", color: !cat ? "#fff" : "var(--ink2)" }}>Tümü</button>
          {CATEGORIES.map((c) => (
            <button key={c.id} onClick={() => setCat(cat === c.id ? null : c.id)} className="press text-left rounded-lg px-2.5 py-1.5 fontui inline-flex items-center gap-2" style={{ fontSize: 13, background: cat === c.id ? "var(--ink)" : "transparent", color: cat === c.id ? "#fff" : "var(--ink2)" }}><c.icon size={14} />{c.label}</button>
          ))}
        </div>
      </div>
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          {BRANDS.map((b) => { const on = brandFilter.includes(b); return (
            <button key={b} onClick={() => setBrandFilter((f) => f.includes(b) ? f.filter((x) => x !== b) : [...f, b])} className="press flex items-center gap-2 fontui" style={{ fontSize: 13, color: "var(--ink2)" }}>
              <span className="flex items-center justify-center" style={{ width: 18, height: 18, borderRadius: 6, background: on ? "var(--ink)" : "#fff", border: on ? "none" : "1.5px solid #D4D8DF" }}>{on && <Check size={12} color="#fff" />}</span>{b}
            </button> ); })}
        </div>
      </div>
      <div>
  ...
  const SortDropdown = () => (
    <div className="relative">
      <button onClick={() => setSortOpen((v) => !v)} className="press inline-flex items-center gap-1.5 rounded-xl px-3 py-2 bg-white" style={{ fontSize: 13, border: "1px solid var(--line)", color: "var(--ink2)", fontWeight: 600 }}><ArrowUpDown size={14} /> {sort} <ChevronDown size={14} /></button>
      {sortOpen && (
        <div className="absolute right-0 z-30 mt-1 bg-white shadowcard rounded-xl overflow-hidden" style={{ border: "1px solid var(--line)", width: 170 }}>
          {["Önerilen", "En düşük fiyat", "En yüksek fiyat", "En yeni", "Puan"].map((o) => <button key={o} onClick={() => { setSort(o); setSortOpen(false); }} className="press w-full text-left fontui px-3 py-2" style={{ fontSize: 13, background: sort === o ? "#F4F5F7" : "#fff", color: sort === o ? "var(--ink)" : "var(--ink2)", fontWeight: sort === o ? 700 : 500 }}>{o}</button>)}
        </div>
      )}
    </div>
  );
  ...
  const Empty = ({ icon: Icon, title, desc, cta, onCta }) => (
    <div className="flex flex-col items-center justify-center text-center gap-2.5 py-20">
      <div className="flex items-center justify-center" style={{ width: 56, height: 56, borderRadius: 18, background: "#fff", border: "1px solid var(--line)", color: "#AEB4BE" }}><Icon size={24} /></div>
      ...
      {cta && <button onClick={onCta} className="press mt-1 bgink text-white fontui font-semibold rounded-xl px-4 py-2" style={{ fontSize: 13 }}>{cta}</button>}
    </div>
  );
      ...
      <div className="anim">
        ...
        <section className="relative overflow-hidden rounded-3xl mt-4 lg:mt-6 px-5 py-8 lg:px-12 lg:py-14" style={{ background: "linear-gradient(135deg,#0E1218 0%,#161D27 60%,#10302A 130%)" }}>
          <div className="max-w-2xl">
            ...
            <div className="mt-6 flex items-center gap-2 bg-white rounded-2xl p-1.5 pl-4 max-w-xl shadow-lg">
              ...
              <button onClick={runSearch} className="press bgink text-white fontui font-semibold rounded-xl px-5 py-2.5" style={{ fontSize: 14 }}>Ara</button>
            </div>
          </div>
        ...
        <section className="mt-8">
          ...
          <div className="grid grid-cols-3 lg:grid-cols-6 gap-2.5">
            {CATEGORIES.map((c) => (
              <button key={c.id} onClick={() => { setCat(c.id); setSearchTerm(""); go("results"); }} className="press bg-white border bd rounded-2xl flex flex-col items-center justify-center gap-2 py-4 shadowcard">
  ...
  const PageResults = () => (
    <div className="anim mt-4 lg:mt-6">
      ...
      <div className="flex gap-6 items-start">
        <aside className="hidden lg:block shrink-0 sticky top-24" style={{ width: 220 }}><div className="bg-white border bd rounded-2xl p-4 shadowcard"><Filters /></div></aside>
    ...
    return (
      <div className="anim mt-4 lg:mt-6">
        ...
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-10 items-start">
          {/* SOL: görsel */}
          <div className="bg-white border bd rounded-3xl p-6 flex items-center justify-center shadowcard" style={{ minHeight: isDesktop ? 380 : 220 }}>
          ...
          <div>
            ...
            <div className="mt-4 border bd rounded-2xl overflow-hidden">
              {sorted.map((s, i) => (
                <div key={s.name} className="flex items-center justify-between px-4 py-3" style={{ borderTop: i ? "1px solid var(--line2)" : "none", background: i === 0 ? "var(--saveBg)" : "#fff" }}>
            ...
            <div className="flex gap-2.5 mt-4">
              ...
              <button onClick={() => { if (!compare.includes(p.id)) toggleCompare(p.id); go("compare"); }} className="press flex-1 inline-flex items-center justify-center gap-2 py-3 fontui font-semibold rounded-xl bgink text-white" style={{ fontSize: 14 }}><Scale size={16} /> Karşılaştır</button>
        ...
        {/* TEKNİK ÖZELLİKLER */}
        <section className="mt-8 bg-white border bd rounded-2xl p-5 shadowcard">
  ...
  const PageCompare = () => {
    ...
    return (
      <div className="anim mt-4 lg:mt-6">
        ...
        <div className="bg-white border bd rounded-2xl overflow-hidden shadowcard">
          {/* başlık */}
          <div className="grid sticky top-16 lg:top-20 bg-white z-10" style={{ gridTemplateColumns: "92px " + sel.map(() => colW).join(" "), borderBottom: "1px solid var(--line)" }}>
            <div />
            {sel.map((p, i) => (
            ...
            return (
              <div key={r.k} className="grid" style={{ gridTemplateColumns: "92px " + sel.map(() => colW).join(" "), borderTop: "1px solid var(--line2)" }}>
                ...
                {sel.map((p, i) => { const win = r.better && r.get(p) === best && new Set(vals).size > 1; return (
                  <div key={p.id} className="px-3 py-3" style={{ borderLeft: "1px solid var(--line)", background: win ? "var(--saveBg)" : "#fff" }}>
  ...
  const PageAi = () => {
    ...
    if (aiStep === 0) return wrap(<>
      ...
      <div className="grid sm:grid-cols-2 gap-3">{uses.map((u) => { const on = aiUse === u.id; return (
        <button key={u.id} onClick={() => { setAiUse(u.id); setAiStep(1); }} className="press flex items-center gap-3 bg-white p-4 text-left shadowcard rounded-2xl" style={{ border: on ? "1.5px solid var(--ai)" : "1px solid var(--line)" }}>
    ...
    if (aiStep === 1) return wrap(<>
      ...
      <div className="grid sm:grid-cols-2 gap-3">{budgets.map((bd) => { const on = aiBudget === bd; return (
        <button key={bd} onClick={() => { setAiBudget(bd); startAi(); }} className="press flex items-center justify-between bg-white p-4 shadowcard rounded-2xl" style={{ border: on ? "1.5px solid var(--ai)" : "1px solid var(--line)" }}>
    ...
    return wrap(<>
      ...
      <div className="flex flex-col gap-3">{picks.map((p, i) => (
        <button key={p.id} onClick={() => openProduct(p.id)} className="press bg-white p-4 text-left shadowcard rounded-2xl" style={{ border: i === 0 ? "1.5px solid var(--ai)" : "1px solid var(--line)" }}>
          <div className="flex gap-3">
            <div className="relative shrink-0"><Thumb p={p} className="w-16 h-16" radius={15} /><span className="mono absolute flex items-center justify-center" style={{ top: -7, left: -7, width: 24, height: 24, borderRadius: 24, background: i === 0 ? "var(--ai)" : "var(--ink)", color: "#fff", fontWeight: 700, fontSize: 11, border: "2px solid #fff" }}>{i + 1}</span></div>
            <div className="flex-1 min-w-0">
  ...
  const PageNotif = () => {
    ...
    return (
      <div className="anim mt-4 lg:mt-6 mx-auto" style={{ maxWidth: 680 }}>
        ...
        <div className="bg-white border bd rounded-2xl overflow-hidden shadowcard">
          {items.map((n, i) => (
            <div key={i} className="flex gap-3 px-4 py-3.5" style={{ borderTop: i ? "1px solid var(--line2)" : "none", background: n.unread ? "#fff" : "#FAFBFC" }}>
              <span className={"flex items-center justify-center shrink-0 " + (n.kind === "save" ? "save bgsaveb" : n.kind === "ai" ? "ai bgaib" : "ink")} style={{ width: 40, height: 40, borderRadius: 13, background: n.kind === "ink" ? "#F2F4F7" : undefined }}><n.i size={18} /></span>
  ...
  return (
    <div className="fontui appbg min-h-screen flex flex-col">
      {/* HEADER */}
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b bd">
        <div className="mx-auto max-w-6xl px-4 lg:px-8 h-16 flex items-center gap-4">
          ...
          <div className="hidden md:flex flex-1 items-center gap-2 bg-gray-100 rounded-xl px-3 max-w-xl mx-auto" style={{ height: 42 }}>
      ...
      {/* FOOTER (desktop) */}
      <footer className="hidden lg:block border-t bd bg-white">
      ...
      {/* ALT NAV (mobil) */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t bd flex items-stretch">
      ...
      {sheetOpen && (
        <div className="lg:hidden fixed inset-0 z-40 flex items-end" style={{ background: "rgba(16,21,27,.45)" }} onClick={() => setSheetOpen(false)}>
          <div className="w-full bg-white p-5 anim" style={{ borderTopLeftRadius: 22, borderTopRightRadius: 22, maxHeight: "80%", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
            ...
            <Filters />
            <button onClick={() => setSheetOpen(false)} className="press w-full mt-5 bgink text-white fontui font-semibold rounded-xl py-3" style={{ fontSize: 14 }}>{results.length} sonucu göster</button>
          </div>
        </div>
      )}
      ...
      {alarm && (
        <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center p-0 lg:p-4" style={{ background: "rgba(16,21,27,.5)" }} onClick={() => setAlarm(null)}>
          <div className="w-full bg-white anim" style={{ maxWidth: 440, borderTopLeftRadius: 22, borderTopRightRadius: 22, borderRadius: isDesktop ? 22 : undefined }} onClick={(e) => e.stopPropagation()}>
            {alarmDone ? (
              <div className="p-8 flex flex-col items-center text-center gap-3">
                ...
                <button onClick={() => setAlarm(null)} className="press mt-2 bgink text-white fontui font-semibold rounded-xl px-6 py-2.5" style={{ fontSize: 14 }}>Tamam</button>
              </div>
            ) : (
              <div className="p-5">
                ...
                <div className="flex gap-3 items-center bg-gray-50 border bd rounded-xl p-3 mb-4"><Thumb p={alarm} className="w-11 h-11" radius={12} /><div><p className="fontui font-semibold ink" style={{ fontSize: 14 }}>{alarm.name}</p><p className="mono ink2" style={{ fontSize: 11 }}>Şu an: {tl(cheapestOf(alarm))}</p></div></div>
                ...
                <button onClick={() => setAlarmDone(true)} className="press w-full bgink text-white fontui font-semibold rounded-xl py-3" style={{ fontSize: 15 }}>Alarmı kur</button>
              </div>
            )}
          </div>
        </div>
